import type { TradingSolutionPageMapped } from '@/lib/mapTradingSolutionPage';
import { SETTLEMENT_SOLUTION_FOOTER_CTA_DEFAULT } from '@/lib/settlementSolutionContent';
import type {
 ...
 */
export const PRODUCT_LINE_NAV: { label: string; href: string }[] = [
  { label: '交易方案', href: '/trading-solution' },
  { label: '结算方案', href: '/settlement-solution' },
  ...PRODUCT_LINE_PAGE_ENTRIES.map((e) => ({
    label: e.label,
    href: productLineEntryHref(e.slug),
  })),
];

const GRID_CTA = { label: '了解更多', href: '/contact' } as const;

const IMG = '/trading-solution/front-office.png';

function rows(items: TradingSolutionFeatureRow[]): TradingSolutionFeatureRow[] {
  return items;
}

function grid(items: TradingSolutionGridItem[]): TradingSolutionGridItem[] {
  return items;
}
    ...
    featureRows: rows([
      {
        ...
        variant: 'portfolio',
        lines: [
          '自动化 eKYC：采用先进识别技术，分钟级开户审核，强化“认证”措施，防范洗钱风险。',
          '统一账户管理：一套账户支持股票与虚拟资产，资金内部即时划转，无需重复提交个人信息。',
          '出入金通道：支持 FPS、eDDA、银行转账、TT 汇款等多种法币出入金方式，银行账单自动批量配比与对账。',
        ],
      },
      {
        title: '专业交易系统',
      ...
      },
      {
        ...
        variant: 'risk-radar',
        lines: [
          '统一风控引擎：跨股票与虚拟资产的统一风控监控，包含交易限额、压力测试、Margin Call 监测、实时预估与融资额度管理。',
          '指数与参数管理：支持指数成分股管理、Haircut 设置及风控大盘参数自定义。',
          '批量平台工具：在极端行情下，支持手动或自动的批量平仓操作，有效控制风险。',
        ],
      },
      {
        title: '清算与报表',
        variant: 'api-console',
        lines: [
          '7×24 清结算：专为虚拟资产设计的清算系统，支持全天候交易下的实时对账、仓位于结算单管理。',
